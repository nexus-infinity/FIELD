-- Fractal Contact Intelligence Suite v4.3
-- OPTIMIZED for large contact databases with improved timeout handling

-- IMPROVED CONFIGURATION (more conservative settings)
property batchSize : 20
property defaultTimeoutSeconds : 180
property progressUpdateInterval : 25
property maxRetries : 3

-- Use a variable for the current timeout that can be changed by the user
...
set startupMessage to "🧬 Fractal Contact Intelligence v4.3" & return & return
set startupMessage to startupMessage & "🔧 OPTIMIZATIONS:" & return
set startupMessage to startupMessage & "• Smaller batch sizes (20 contacts)" & return
set startupMessage to startupMessage & "• Longer timeouts (3 minutes)" & return
set startupMessage to startupMessage & "• Better error recovery" & return
set startupMessage to startupMessage & "• More frequent progress updates" & return & return
set startupMessage to startupMessage & "This version handles large contact databases much better!"
...
-- Get contact count with better error handling
display dialog "🔍 Connecting to contact database..." giving up after 2
try
...
on error errMsg
	if errMsg contains "timeout" then
		set timeoutDialog to "⏳ Initial contact access timed out." & return & return
		set timeoutDialog to timeoutDialog & "Your contact database might be very large." & return
		set timeoutDialog to timeoutDialog & "Would you like to try with an even longer timeout?"
		display dialog timeoutDialog buttons {"Cancel", "Try 5 Minutes"} default button 2
		if button returned of result is "Try 5 Minutes" then
			...
			on error
				display dialog "❌ Still unable to access contacts. Your database might be too large for AppleScript processing." buttons {"OK"} with icon stop
				return
			end try
		else
			return
...
-- Processing confirmation with better time estimates
set estimatedMinutes to round ((totalCount / batchSize) * 0.5)
set confirmText to "🧬 Fractal Processing Analysis" & return & return
set confirmText to confirmText & "📊 Total contacts: " & totalCount & return
set confirmText to confirmText & "⚡ Estimated time: " & estimatedMinutes & " minutes" & return
set confirmText to confirmText & "🎯 Processing in batches of " & batchSize & return
set confirmText to confirmText & "⏳ Timeout per batch: " & (round (timeoutSeconds / 60)) & " minutes"
...
-- PHASE 1: EXTRACT ALL CONTACT DATA (with improved error handling)
display dialog "📊 Phase 1: Data Extraction (Optimized)" giving up after 2
set processingStartTime to current date
set currentIndex to 1
set successfulBatches to 0
set failedBatches to 0

repeat while currentIndex ≤ totalCount
	-- Progress updates (more frequent)
	if (currentIndex mod progressUpdateInterval) = 0 or currentIndex = totalCount then
		set progressPercent to round ((currentIndex / totalCount) * 100)
		set elapsedTime to (current date) - processingStartTime
		if currentIndex > 0 then
			set avgTime to (elapsedTime as number) / (currentIndex as number)
			set remainingTime to (avgTime as number) * (totalCount - currentIndex)
		else
			set remainingTime to 0
		end if
		
		set progressText to "📊 Phase 1: Smart Data Extraction" & return & return
		set progressText to progressText & "Progress: " & progressPercent & "% (" & currentIndex & "/" & totalCount & ")" & return
		set progressText to progressText & "Remaining: ~" & my formatTime(remainingTime) & return
		set progressText to progressText & "✅ Successful batches: " & successfulBatches & return
		set progressText to progressText & "❌ Failed batches: " & failedBatches & return
	...
	set retryCount to 0
	set batchSuccess to false
	
	repeat while retryCount < maxRetries and not batchSuccess
		try
			with timeout of timeoutSeconds seconds
				tell application "Contacts"
					repeat with i from currentIndex to batchEnd
						if i > totalCount then exit repeat
						...
						try
							...
							if contactName is "" then
								...
								if contactName is " " then set contactName to "Contact #" & i
							end if
							...
							set contactRecord to {contactIndex:i, contactName:contactName, firstName:firstName, lastName:lastName, phoneNumbers:phoneNumbers, emailAddresses:emailAddresses}
							set contactList to contactList & {contactRecord}
							
						on error contactError
							set errorRecord to {errorIndex:i, errorMessage:contactError}
							set errorList to errorList & {errorRecord}
						end try
					end repeat
				end tell
			end timeout
			
			set batchSuccess to true
			set successfulBatches to successfulBatches + 1
			
		on error batchError
			set retryCount to retryCount + 1
			if retryCount < maxRetries then
				display dialog "⚠️ Batch " & successfulBatches & " failed. Retrying " & retryCount & "/" & maxRetries & "..." giving up after 2
				delay 2
			else
				set failedBatches to failedBatches + 1
				display dialog "❌ Batch " & successfulBatches & " failed after " & maxRetries & " attempts. Continuing..." giving up after 2
				set errorRecord to {errorIndex:currentIndex, errorMessage:"Batch failure: " & batchError}
				set errorList to errorList & {errorRecord}
			end if
		end try
	end repeat
...
set extractedCount to count of contactList
set successRate to round ((extractedCount / totalCount) * 100)
set extractionResultText to "✅ Data Extraction Complete!" & return & return
set extractionResultText to extractionResultText & "📊 Contacts processed: " & totalCount & return
set extractionResultText to extractionResultText & "✅ Successfully extracted: " & extractedCount & return
set extractionResultText to extractionResultText & "📈 Success rate: " & successRate & "%" & return
set extractionResultText to extractionResultText & "❌ Errors encountered: " & (count of errorList)
display dialog extractionResultText giving up after 3

-- PHASE 2: FRACTAL DUPLICATE DETECTION
display dialog "🧬 Phase 2: Fractal duplicate detection starting..." giving up after 2

-- Pass 1: Exact phone matches
set phoneStartTime to current date
display dialog "⚡ Pass 1: Finding exact phone duplicates..." giving up after 2
...
display dialog "✅ Pass 1 Complete: " & (count of exactPhoneDuplicates) & " phone duplicates found in " & my formatTime(phoneTime as number) giving up after 2

-- Pass 2: Exact email matches
set emailStartTime to current date
display dialog "⚡ Pass 2: Finding exact email duplicates..." giving up after 2
...
display dialog "✅ Pass 2 Complete: " & (count of exactEmailDuplicates) & " email duplicates found in " & my formatTime(emailTime as number) giving up after 2

-- Pass 3: Remove obvious duplicates from processing
set reductionStartTime to current date
display dialog "🔬 Pass 3: Optimizing dataset..." giving up after 2
...
set reductionTime to (current date) - reductionStartTime
display dialog "✅ Pass 3 Complete: Dataset reduced by " & reductionPercent & "% (" & reducedCount & " contacts remaining)" giving up after 2

-- Pass 4: Fuzzy name matching on reduced dataset
set nameStartTime to current date
display dialog "🧠 Pass 4: Fuzzy name analysis..." giving up after 2
...
display dialog "✅ Pass 4 Complete: " & (count of nameMatchDuplicates) & " name duplicates found in " & my formatTime(nameTime as number) giving up after 2

-- Generate reports
display dialog "📊 Generating enhanced reports..." giving up after 2
...
set resultText to resultText & "📊 Total processing time: " & my formatTime(totalProcessingTime as number) & return
set resultText to resultText & "🔍 Dataset reduction: " & reductionPercent & "%" & return
set resultText to resultText & "🎯 Duplicates found: " & totalDuplicates & return
set resultText to resultText & "✅ Success rate: " & successRate & "%" & return & return
set resultText to resultText & "⚡ PHASE BREAKDOWN:" & return
set resultText to resultText & "• Phones: " & my formatTime(phoneTime as number) & " → " & (count of exactPhoneDuplicates) & " duplicates" & return
set resultText to resultText & "• Emails: " & my formatTime(emailTime as number) & " → " & (count of exactEmailDuplicates) & " duplicates" & return
set resultText to resultText & "• Names: " & my formatTime(nameTime as number) & " → " & (count of nameMatchDuplicates) & " duplicates" & return & return
set resultText to resultText & "📁 Reports saved to session folder"
...
-- Remove obvious duplicates from dataset
...
-- Fuzzy name matching on reduced dataset
...
on calculateNameSimilarity(str1, str2)
	...
	return round ((matches / maxLen) * 100)
end calculateNameSimilarity

-- Generate fractal report
on generateFractalReport(contactList, duplicateList, errorList, startTime, phoneTime, emailTime, nameTime, reductionPercent)
	...
	set report to report & "    FRACTAL CONTACT INTELLIGENCE REPORT v4.3" & return
	...
	set report to report & "   Dataset reduction achieved: " & reductionPercent & "%" & return & return
	...
	set report to report & "🔍 DUPLICATE ANALYSIS" & return
	set report to report & "   • Exact phone matches: " & phoneCount & return
	set report to report & "   • Exact email matches: " & emailCount & return
	set report to report & "   • Fuzzy name matches: " & nameCount & return
	set report to report & "   TOTAL DUPLICATES: " & (count of duplicateList) & return & return
	
	set report to report & "🎯 PRIORITY BREAKDOWN" & return
	set report to report & "   • CRITICAL (exact matches): " & criticalCount & return
	set report to report & "   • HIGH (very likely): " & highCount & return
	set report to report & "   • MEDIUM (probable): " & mediumCount & return & return
	
	if (count of errorList) > 0 then
		set report to report & "⚠️ PROCESSING NOTES" & return
		repeat with errorItem in errorList
			set report to report & "   Contact #" & (errorIndex of errorItem) & ": " & (errorMessage of errorItem) & return
		end repeat
	end if
	
	set report to report & return & "Report generated: " & (current date) & return
...
on generateMergeActions(duplicateList)
	...
	set actions to actions & "     FRACTAL PRIORITY MERGE ACTIONS v4.3" & return
	set actions to actions & "═══════════════════════════════════════════════" & return & return
	
	set actions to actions & "🧬 PROCESSING ORDER:" & return
	set actions to actions & "• CRITICAL = Exact matches (process first)" & return
	set actions to actions & "• HIGH = Very likely matches" & return
	set actions to actions & "• MEDIUM = Probable matches" & return & return
	...
	repeat with dup in sortedDuplicates
		...
		set actions to actions & return & "YOUR DECISION:" & return
		set actions to actions & "[ ] MERGE CONTACTS" & return
		set actions to actions & "[ ] KEEP BOTH" & return
		set actions to actions & "[ ] DELETE ONE" & return
		set actions to actions & return & "NOTES: ________________________________" & return & return
